...
public class PostgresPollerProviderIntegrationTest extends PostgresContainerTest
  ...
  private static final String TABLE_NAME = "JdbcPoller";

  public PostgresPollerProviderIntegrationTest() {
    ...
    this.provider = new PostgresPollerProvider.Builder<>(
            this.connection,
    ...
    this.connection.withHandle(h -> h
        ...
        .execute(PostgresPollerProvider.CREATE_TABLE));
  }

  @BeforeEach
  public void beforeEach() {
    this.connection.useHandle(h -> h.execute("TRUNCATE  %1$s".formatted(TABLE_NAME)));
  }
