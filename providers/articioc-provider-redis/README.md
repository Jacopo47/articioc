...
| `addArgument(XAddArgs)`              | Advanced static arguments for the `XADD` command, like `maxlen()` for capping the stream.                                                  | No extra arguments.                                                               |
| `addArgument(Function<A, XAddArgs>)` | A function to dynamically generate `XADD` arguments based on the `Leaf` being written. This will always win over the static `addArgument`. | No extra arguments.                                                               |